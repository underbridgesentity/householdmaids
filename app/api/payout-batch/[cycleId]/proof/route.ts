...
/** ADMIN-only: streams a payout batch's proof-of-payment. */
export async function GET(_req: Request, { params }: { params: Promise<{ cycleId: string }> }) {
  ...
  const cycle = await prisma.payoutCycle.findUnique({ where: { id: cycleId }, select: { proofKey: true } });
  if (!cycle?.proofKey) return new Response("Not found", { status: 404 });

  const bytes = await readDocument(cycle.proofKey);
  const contentType = contentTypeFor(cycle.proofKey);
